...
Issue #73：首次真实作者参考作业在引擎启动前暴露 Slurm 记录解析错误：`SubmitLine`
包含带空格的提交命令，旧实现把每个空白片段当字段而拒绝有效记录。现在按字段边界解析，
保留重复字段、多条记录、任务身份、资源、用户和时间核对；合成用例覆盖含参数的提交行
以及其中出现伪造身份字段时的拒绝。此次失败计入真实提交历史，未进行目标物理计算。
下文初始部署说明为历史能力边界，不应将此次调度作业当作运行器或科学验收已通过。
