...
成本尚无 HPC 实测，不能给可信核时或费用预测。用户已于 2026-09-23 批准以下**有上限的开发验证**：单作业最多 8 核 × 30 分钟 = 4 核时、内存最多 8 GiB、并发 1、存储最多 1 GiB；初阶段总上限 12 核时（参考及 Agent 计算各自记账）。这是已批准的停止上限，不是耗时估计或完成承诺；不能为塞入上限缩小原任务。模型调用上限暂为 0。完整任务若超过此上限，应停止并重新提出预算。

用户已确认原创代码采用 Apache-2.0。上述预算和许可已记录；具体复现范围、条件与评分规则尚未冻结。模型调用额度保持为 0，未获得收费 API 调用授权。
